...
const AlbumDetailsPage = () => {
  ...
  // Album Customer Photos (Up to sheet_count * 2 photos, e.g. 20 sheets = 40 photos)
  const [albumPhotos, setAlbumPhotos] = useState([]);
  const [uploadingAlbumPhotos, setUploadingAlbumPhotos] = useState(false);

  // Calculate sheets and photo capacity (1 sheet = 2 pages/photos)
  const sheetCount = useMemo(() => {
    if (!album) return 20;
    const directSheets = Number(album.sheet_count);
    if (directSheets > 0) return directSheets;
    const directPages = Number(album.total_pages);
    if (directPages > 0) return Math.round(directPages / 2);
    return 20;
  }, [album]);

  const maxPhotos = useMemo(() => {
    if (!album) return 40;
    return (sheetCount * 2) || Number(album.total_pages) || 40;
  }, [album, sheetCount]);
