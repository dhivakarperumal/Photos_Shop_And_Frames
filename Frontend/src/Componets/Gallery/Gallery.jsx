...
const Gallery = () => {
  ...
  return (
    <main className="bg-[#fffdfa] text-[#101d35]">
      ...
      <section id="gallery-grid" className="pt-16"><PageContainer><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><p className="text-xs font-semibold text-[#303b4b]">Showing {photos.length ? `1-${Math.min(visibleCount, photos.length)}` : 0} of {photos.length} photos</p><div className="flex items-center gap-2"><label className="flex items-center gap-2 rounded-md border border-[#e0ddd9] bg-white px-3 py-2 text-xs text-[#4b5563]">Sort by:<select value={sort} onChange={(event) => setSort(event.target.value)} className="bg-transparent font-semibold outline-none"><option>Latest</option><option>Oldest</option></select><ChevronDown className="h-3.5 w-3.5" /></label><button type="button" className="hidden h-9 w-9 items-center justify-center rounded-md bg-[#102139] text-white sm:flex" aria-label="Grid view"><Grid2X2 className="h-4 w-4" /></button><button type="button" className="hidden h-9 w-9 items-center justify-center rounded-md border border-[#dedbd7] text-[#667085] sm:flex" aria-label="List view"><List className="h-4 w-4" /></button></div></div>
        {loading ? <div className="flex justify-center py-24"><Loader2 className="h-8 w-8 animate-spin text-[#ed741d]" /></div> : photos.length === 0 ? <div className="rounded-2xl border border-[#eadfd5] bg-[#faf7f3] py-24 text-center text-sm text-[#647080]">No gallery photos available yet.</div> : <div className={viewMode === "grid" ? "grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5" : "grid gap-3 sm:grid-cols-2"}>{photos.slice(0, visibleCount).map((photo) => <button key={photo.id} type="button" onClick={() => setSelectedPhoto(photo)} className={viewMode === "grid" ? "group relative aspect-square overflow-hidden rounded-md bg-[#eee7df] text-left" : "group flex items-center gap-4 overflow-hidden rounded-xl border border-[#eee8e1] bg-white p-2 text-left"}><img src={photo.src} alt={photo.title} loading="lazy" className={viewMode === "grid" ? "h-full w-full object-cover transition duration-500 group-hover:scale-105" : "h-24 w-28 shrink-0 rounded-lg object-cover transition duration-500 group-hover:scale-105"} /><span className={viewMode === "grid" ? "absolute inset-x-0 bottom-0 translate-y-full bg-black/65 px-3 py-2 text-[10px] font-semibold text-white transition group-hover:translate-y-0" : "text-sm font-bold text-[#263448]"}>{photo.title}</span></button>)}</div>}
