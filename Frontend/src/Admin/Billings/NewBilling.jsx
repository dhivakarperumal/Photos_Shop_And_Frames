...
import { ChevronDown, Minus, Plus, Printer, Search, Trash2 } from "lucide-react";
...
const NewBilling = () => {
  ...
  const addSelectedProduct = () => {
    ...
    const item = {
      ...
      price: Number(variant?.offer_price ?? variant?.selling_price ?? variant?.mrp ?? product.price),
    };

    setItems((current) => {
      const existing = current.find((currentItem) => String(currentItem.id) === String(item.id));
      if (existing) {
        return current.map((currentItem) => String(currentItem.id) === String(item.id)
    ...
    <div className="min-h-screen bg-[#f3f4f6] p-4 text-[#1f2937] md:p-6">
      <div className="mx-auto max-w-[1500px]">
        ...
        <div className="grid gap-3 xl:grid-cols-2">
          ...
          <section className="rounded-lg border border-[#e5e7eb] bg-white p-3"><h2 className="mb-4 text-sm font-bold">Customer Details</h2><div className="grid gap-3 md:grid-cols-2"><label className="text-[10px] font-semibold">Name *<input placeholder="Enter customer name" className={fieldClass} /></label><label className="text-[10px] font-semibold">Email<input type="email" placeholder="Enter email address" className={fieldClass} /></label><label className="text-[10px] font-semibold">Phone *<input placeholder="Enter phone number" className={fieldClass} /></label><label className="text-[10px] font-semibold">Address<textarea placeholder="Enter complete address" className="mt-1 h-14 w-full resize-none rounded-md border border-[#e5e7eb] px-3 py-2 text-xs outline-none focus:border-[#ff8a4c]" /></label></div></section>
        </div>

        <section className="mt-3 rounded-lg border border-[#e5e7eb] bg-white p-3"><div className="mb-3 flex items-center justify-between"><h2 className="text-sm font-bold">Order Items</h2><div className="flex gap-2"><div className="relative hidden sm:block"><Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[#6b7280]" /><input placeholder="Search products..." className="h-9 rounded-md border border-[#e5e7eb] pl-8 pr-3 text-xs outline-none" /></div><button type="button" className="rounded-md border border-[#ff9869] px-3 py-2 text-xs font-semibold text-[#ed6b26]">+ Add Item</button><button type="button" onClick={() => setItems([])} className="rounded-md border border-[#ffb3b3] px-3 py-2 text-xs font-semibold text-[#d04d4d]">Clear All</button></div></div><div className="overflow-x-auto"><table className="min-w-full text-left text-[11px]"><thead><tr className="bg-[#fff4ed] font-semibold"><th className="rounded-tl-md px-2 py-3">#</th><th className="px-2 py-3">Product</th><th className="px-2 py-3">Category</th><th className="px-2 py-3">Price (₹)</th><th className="px-2 py-3">Qty</th><th className="px-2 py-3">Discount (₹)</th><th className="px-2 py-3">Total (₹)</th><th className="rounded-tr-md px-2 py-3">Action</th></tr></thead><tbody>{items.map((item, index) => <tr key={item.id} className="border-b border-[#f0f1f3]"><td className="px-2 py-3">{index + 1}</td><td className="px-2 py-3 font-semibold">{item.name}<div className="font-normal text-[#6b7280]">{item.detail}</div></td><td className="px-2 py-3">{item.category}</td><td className="px-2 py-3"><input value={item.price.toFixed(2)} readOnly className="h-9 w-24 rounded-md border border-[#e5e7eb] px-2 text-right text-xs" /></td><td className="px-2 py-3"><div className="flex items-center"><button type="button" onClick={() => updateQuantity(item.id, -1)} className="flex h-8 w-8 items-center justify-center border border-[#e5e7eb]"><Minus className="h-3 w-3" /></button><span className="flex h-8 w-9 items-center justify-center border-y border-[#e5e7eb]">{item.quantity}</span><button type="button" onClick={() => updateQuantity(item.id, 1)} className="flex h-8 w-8 items-center justify-center border border-[#e5e7eb]"><Plus className="h-3 w-3" /></button></div></td><td className="px-2 py-3"><input value={item.discount.toFixed(2)} readOnly className="h-9 w-24 rounded-md border border-[#e5e7eb] px-2 text-right text-xs" /></td><td className="px-2 py-3 font-semibold">{money(item.price * item.quantity)}</td><td className="px-2 py-3"><button type="button" onClick={() => removeItem(item.id)} className="rounded-md border border-[#ffb3b3] p-2 text-[#d04d4d]"><Trash2 className="h-3.5 w-3.5" /></button></td></tr>)}</tbody></table></div></section>

        <div className="mt-3 grid gap-3 xl:grid-cols-[1fr_1fr_0.85fr]"><section className="rounded-lg border border-[#e5e7eb] bg-white p-3"><h2 className="text-sm font-bold">Order Notes</h2><textarea placeholder="Enter order notes (optional)" className="mt-3 h-24 w-full resize-none rounded-md border border-[#e5e7eb] p-3 text-xs outline-none focus:border-[#ff8a4c]" /></section><section className="rounded-lg border border-[#e5e7eb] bg-white p-3"><h2 className="text-sm font-bold">Discount &amp; Charges</h2><div className="grid gap-3 sm:grid-cols-2"><label className="text-[10px] font-semibold">Discount Type<select className={fieldClass}><option>Flat Discount</option><option>Percentage</option></select></label><label className="text-[10px] font-semibold">Discount Amount (₹)<input value={discount} onChange={(event) => setDiscount(event.target.value)} className={fieldClass} /></label><label className="text-[10px] font-semibold">Shipping Charge (₹)<input value={shippingCharge} onChange={(event) => setShippingCharge(event.target.value)} className={fieldClass} /></label><label className="text-[10px] font-semibold">Packaging Charge (₹)<input value={packagingCharge} onChange={(event) => setPackagingCharge(event.target.value)} className={fieldClass} /></label></div></section><section className="rounded-lg border border-[#e5e7eb] bg-white p-3 text-xs"><div className="space-y-2"><p className="flex justify-between">Subtotal <b>{money(subtotal)}</b></p><p className="flex justify-between">Discount <b className="text-[#198754]">- {money(discount)}</b></p><p className="flex justify-between">Shipping Charge <b>{money(shippingCharge)}</b></p><p className="flex justify-between">Packaging Charge <b>{money(packagingCharge)}</b></p><p className="flex justify-between">Tax (18%) <b>{money(0)}</b></p><p className="flex justify-between border-t border-[#e5e7eb] pt-3 text-sm font-bold">Total Amount <strong className="text-lg text-[#ed5d19]">{money(total)}</strong></p></div></section></div>
